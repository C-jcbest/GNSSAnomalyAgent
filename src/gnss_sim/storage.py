...
class DatasetStore:
    ...
    def _new_manifest(self, request: GenerationRequest) -> DatasetManifest:
        ...
        dataset_id = f"sim-{created_at:%Y%m%d-%H%M%S}-{uuid4().hex[:8]}"
        encoded = json.dumps(request.model_dump(mode="json"), sort_keys=True).encode("utf-8")
    ...
    def _run(self, manifest: DatasetManifest) -> DatasetManifest:
        ...
        try:
            for index in range(manifest.request.count):
                ...
                case_input, truth = generate_normal_case(
                    case_id, case_seed, manifest.request.config
                )
