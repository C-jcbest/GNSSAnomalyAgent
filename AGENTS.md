...
- P4 已冻结 `pilot-v1`（seed 20260925，30 Normal/120 Single/150 Multi）；后续方法统一使用该批输入，不按曲线或检测表现筛选、重抽。`events[]` 是唯一真值，评分忽略预测 type，严格轴与一对一匹配，失败保留分母。规则以 `docs/05-p4-pilot-evaluator.md` 为准；P5 前不加检测器或模型调用。
