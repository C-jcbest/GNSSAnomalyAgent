...
真值分别保存 `normal_background_mm`、`measurement_noise_mm`、`injected_deformation_mm` 和 `observation_artifact_mm`。正常例的后两项为全零。模拟器的 `source` 只表示注入数组，不是检测器必须判断的现场成因；未来检测结果只需报告异常日期或活动区间及其轴。Step、Slow Trend 和 Acceleration 注入形变；Spike 与 Transient Shift 注入观测伪差。
...
Slow Trend 与 Acceleration 的 `final_offset_mm` 保存带符号终值。
