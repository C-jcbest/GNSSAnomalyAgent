...
> 版本：2026-09-25 · `event-v6` 生成器及 `pilot-v1`/P4 事件评价器已实现。P5～P9 尚未实施；目前没有检测器或性能结果。
...
| P4 | 300 例固定开发 Pilot、从 `events[]` 派生状态视图、手工预测校验事件评价器 | 已实现；见 [P4 协议](05-p4-pilot-evaluator.md) |
...
先看《P1 正常序列模型》理解背景，再看《P2 事件与 P3 场景协议》理解五种事件与六种组合，最后看《数据契约与可复现流程》核对 seed、文件和接口。网页的真值视图只用于生成检查；未来检测方法只能使用案例输入。
