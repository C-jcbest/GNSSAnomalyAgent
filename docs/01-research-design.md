...
> 版本：2026-09-24 · 当前统一生成协议为 `event-v4`，含正常背景和 P2 单事件。P3～P9 是研究路线；目前没有检测器或性能结果。

## 摘要

本实验以合成的日尺度 N（北）、E（东）、U（高）三轴坐标为共同输入，逐步研究异常信号的生成、定位与评价。模拟器保存已知成分；后续检测器只能读取观测输入，并报告可观察形态、分量与日期区间。曲线形态本身不能裁决仪器故障、真实地表形变或滑坡成因。

P1 的职责是提供一个**固定、可复现的正常背景**，不是研究贡献或万能仿真器。每例恰有 365 个日期，覆盖 2025-01-01 至 2025-12-31。当前 P2 的公开生成控制项为案例类型、数据集 seed 和案例数；事件幅值、时长及位置不开放。改变背景模型、噪声类别或窗口长度都要另立实验版本。
...
| 阶段 | 目标 | 状态 |
| --- | --- | --- |
| P1 | 365 日固定正常背景、输入与真值隔离、可复现生成及交互浏览 | 已纳入 `event-v4` |
| P2 | 五种单轴单事件注入、typed truth、独立贡献与交互核查 | 已实现；`event-v4` |
